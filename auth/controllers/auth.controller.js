...
exports.guardarCarritoSkill2 = async(req, res, next) => {
    try {
        const token = req.params.token;
        const number = req.params.number; // Obtener el ID del comensal de los parámetros de la URL
        const { carrito } = req.body; // Obtener el carrito del cuerpo de la solicitud

        // Buscar al comensal por su token
        const comensal = await User.findOne({ token: token, telefono: number});
